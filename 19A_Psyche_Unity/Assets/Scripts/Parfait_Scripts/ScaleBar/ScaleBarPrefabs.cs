...
public class ScaleBarPrefabs : MonoBehaviour
{
    #region FIELDS
    
    public static ScaleBarPrefabs singleton;
    
    [System.Serializable]
    public class ScaleBarPrefab // Describes a Scalebar object. Includes the prefab, the height value in real world (in meters), and the unit 
    {
    ...
    [HideInInspector]
    public bool scalebarMode = false; // determines if user has scalebar tool enabled or not
    public List<ScaleBarPrefab> bars; // list containing all of the scalebar prefabs. When the game starts, this list gets populated with the needed data.
    ...
    private float scale; // calculation of the scale, determined by the terrain's UnityUnitPerMeter and the scalebar's height in real world
    
    [Header("Terrain")]
    public GameObject terrain; // terrain GameObject
    //public SampleTerrainsMenu _SampleTerrainsMenu; // used to determine when a sample terrain has been loaded so we can refresh the scalebar values for the new terrain
    ...
    private bool pressedThisFrame; // determines if VR user pressed button or not for that frame. Used to make code happen once, and not every frame.
    private bool changeTextOnce = false; // ensures that the Scalebar's text gets changed once, and not every frame, for both desktop and VR 
    private bool placeOnce = false; // ensures that a Scalebar is spawned into a scene once. Prevents scalebars spawning every frame in VR
    ...
    void Update()
    {
        ...
        if (scalebarMode)
        {
            ...
            //     index += 1;
            //     if (index<0 || index >= bars.Count) { index = 0; } // makes sure index does not exceed array length
            //     Destroy(obj);
            ...
            scale = (bars[index].realWorldHeight * SceneMaterializer.singleton.UnityUnitPerMeter) * terrain.transform.parent.localScale.x;
    ...
    public void CalculatePrefabs(JMARSScene scene)
    {
        ...
        for (int i = 0; i < xxdim; i+=5)
        {
            if ((i * SceneMaterializer.singleton.UnityUnitPerMeter) * terrain.transform.parent.localScale.x > terrain.transform.parent.localScale.x 
                || (i * SceneMaterializer.singleton.UnityUnitPerMeter) * terrain.transform.parent.localScale.x > 100)
            {
                // round down to value so we don't get messy numbers displayed on the scalebar (ex: 23,947 m)
                if (i % valueToRoundTo == 0)
                {
    ...
    public void CalculateRay()
    {
        ...
        for (float t = 0.01f; t < 1000; t += step)
        {
            ...
            if (ht_minus_rty == 0 || lastSign == -(Mathf.Sign(ht_minus_rty)))
            {
                ...
                {
                    Vector3 intersectionPoint = terrain.transform.TransformPoint(ray_t); // x y z position of where user is intersecting terrain]
                    ...
                    obj.transform.localEulerAngles = new Vector3(0, obj.transform.localEulerAngles.y, 0); // only rotate in the yaw axis
