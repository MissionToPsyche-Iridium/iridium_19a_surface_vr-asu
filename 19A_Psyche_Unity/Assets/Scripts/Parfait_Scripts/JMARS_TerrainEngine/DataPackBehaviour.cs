        ...
        public void LoadByteData()
        {
            try
            {
                foreach (string f in Directory.GetFiles(Application.dataPath +
                                                      $"/Resources/JMARS_Sample_Terrains/{gameObject.name}"))
                {
                    print(Path.GetExtension(f));
                    if (Path.GetExtension(f) == ".raw")
                    {
                        byte[] bytes = File.ReadAllBytes(f);
                        File.WriteAllBytes(Application.dataPath +$"/Resources/JMARS_Sample_Terrains/{gameObject.name}/{Path.GetFileNameWithoutExtension(f)}.txt", bytes);
                    }
                }
            }
            catch(Exception e)
